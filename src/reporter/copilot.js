...
function buildAnalysisMd(result) {
  ...
  const problematic = result.results.filter(
    (t) => t.classification !== "stable_pass"
  );

  const sections = [
    `# Flaky Test Analysis — Workspace Context`,
    ``,
    `> This file provides structured context for GitHub Copilot. Keep it open as workspace context when using Copilot Chat or Agent Mode to investigate test failures.`,
    ``,
    `## Summary`,
    ``,
    `| Metric | Value |`,
  ...
  if (problematic.length === 0) {
    ...
    sections.push(`All ${s.totalTests} tests pass consistently across all runs. No issues to investigate.`);
...
function buildTestContext(test, stats) {
  ...
  lines.push(`- **History:** ${test.history.map((h) => outcomeLabel(h)).join(" → ")} (${test.runCount} runs)`);
  ...
  if (stats && stats.failureFrequency) {
    ...
    if (freq) {
      lines.push(`**Failure Frequency:** ${freq.failureCount}/${freq.totalRuns} runs (${freq.failureRate}%)`);
      lines.push(``);
    }
  }
...
function buildStatsContext(stats) {
  ...
  if (stats.slowestTests && stats.slowestTests.length > 0) {
    ...
    for (const t of stats.slowestTests.slice(0, 5)) {
      lines.push(`- **${t.title}**: ${t.totalDuration}ms total, ${t.maxDuration}ms max, ${t.retries} retries`);
    }
    lines.push(``);
  }
  ...
  if (stats.failureCategories && stats.failureCategories.total > 0) {
    ...
    for (const cat of Object.keys(fc.counts).sort(
      (a, b) => fc.counts[b] - fc.counts[a]
    )) {
...
function buildPromptsMd(result) {
  ...
  const sections = [
    `# Copilot Prompts — Flaky Test Investigation`,
    ``,
    `> Copy-paste these prompts into GitHub Copilot Chat, Agent Mode, or use them as workspace instructions.`,
    ...
    ``,
    `Use these in Copilot Chat (Ctrl+I or the Chat panel) after opening \`analysis.md\` as context.`,
    ``,
  ];
  ...
  sections.push(
    ...
    `## Agent Mode Prompts`,
    ``,
    `Use these in Copilot Agent Mode (Ctrl+Shift+I or the Agent panel) for autonomous investigation and fixes. Agent Mode can read files, run commands, and make edits.`,
    ``
  );
  ...
  sections.push(
    ...
    `## Workspace Prompts`,
    ``,
    `Use these as workspace instructions (\`.github/copilot-instructions.md\`) or project-level Copilot configuration.`,
    ``
  );
