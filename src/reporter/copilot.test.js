...
const { generate } = require("./copilot");
...
function makeResult(overrides = {}) {
  return {
    ...
    statistics: {
      ...
      slowestTests: [
        { title: "Login > should show error for invalid credentials", totalDuration: 5000, maxDuration: 3000, retries: 2 },
        { title: "Dashboard > should load metrics", totalDuration: 3200, maxDuration: 2000, retries: 1 },
      ],
      failureFrequency: [
        { title: "Dashboard > should load metrics", failureCount: 3, totalRuns: 3, failureRate: 100 },
        { title: "Login > should show error for invalid credentials", failureCount: 2, totalRuns: 3, failureRate: 66.67 },
        { title: "Search > returns results", failureCount: 1, totalRuns: 3, failureRate: 33.33 },
        { title: "Profile > should update avatar", failureCount: 1, totalRuns: 3, failureRate: 33.33 },
      ],
      failureCategories: {
        total: 4,
        counts: { timeout: 1, locator: 1, assertion: 1, network: 1, authentication: 0, environment: 0, data: 0, unknown: 0 },
        errors: [],
      },
      browserStats: [
        { browser: "chromium", totalTests: 10, totalFailures: 6, totalFlaky: 1, totalRetries: 3, failRate: 60, flakyRate: 10 },
        { browser: "firefox", totalTests: 5, totalFailures: 3, totalFlaky: 0, totalRetries: 1, failRate: 60, flakyRate: 0 },
      ],
      schemaVersion: "1.0.0",
    },
    runs: [],
...
describe("copilot — generate", () => {
  ...
  it("COPILOT_PROMPTS.md starts with header", () => {
    const output = generate(makeResult());
    assert.ok(output.promptsMd.startsWith("# Copilot Prompts"));
  });

  it("includes Copilot Chat section", () => {
    const output = generate(makeResult());
    assert.ok(output.promptsMd.includes("## Copilot Chat Prompts"));
...
describe("copilot — all stable", () => {
  it("shows all-stable message and minimal content", () => {
    const result = makeResult({
      ...
      statistics: {
        ...
        slowestTests: [],
        failureFrequency: [],
        failureCategories: { total: 0, counts: { timeout: 0, locator: 0, assertion: 0, network: 0, authentication: 0, environment: 0, data: 0, unknown: 0 }, errors: [] },
        browserStats: [],
        schemaVersion: "1.0.0",
      },
    });
  ...
  it("generates Copilot prompts even when all tests pass", () => {
    const result = makeResult({
      summary: {
        runsAnalyzed: 2,
        totalTests: 2,
      ...
      statistics: {
        runs: 2,
        perRun: [],
        aggregate: { overallPassRate: 100, overallFailRate: 0, avgDurationAcrossRuns: 100, avgRetriesAcrossRuns: 0, bestRunPassRate: 100, worstRunPassRate: 100 },
        slowestTests: [],
        failureFrequency: [],
        failureCategories: { total: 0, counts: { timeout: 0, locator: 0, assertion: 0, network: 0, authentication: 0, environment: 0, data: 0, unknown: 0 }, errors: [] },
        browserStats: [],
  ...
  it("handles string errors in classifiedErrors", () => {
    const result = makeResult({
      ...
      results: [
        {
          ...
          classifiedErrors: [
            { message: "network error", category: "network", stack: null },
          ],
          runCount: 2,
        },
      ],
      statistics: {
        ...
        slowestTests: [],
        failureFrequency: [],
        failureCategories: { total: 1, counts: { timeout: 0, locator: 0, assertion: 0, network: 1, authentication: 0, environment: 0, data: 0, unknown: 0 }, errors: [] },
        browserStats: [],
        schemaVersion: "1.0.0",
      },
    });
  ...
  it("does not include regression prompts when no regressions exist", () => {
    const result = makeResult({
      ...
      statistics: {
        ...
        slowestTests: [],
        failureFrequency: [],
        failureCategories: { total: 1, counts: { timeout: 1, locator: 0, assertion: 0, network: 0, authentication: 0, environment: 0, data: 0, unknown: 0 }, errors: [] },
        browserStats: [],
        schemaVersion: "1.0.0",
      },
    });
